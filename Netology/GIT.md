...
- Удалить удаленну (в удаленном репозитории) ветку
  ```
  git push -d origin devops_vm_graylog
  ```
...
- Создать,связать удаленную ветку с локальной и провалится в нее
  ...
  ```
 - Перенести ветку из одного репозитория в другой.Для этого надо удалить ссылку на удаленный репозиторий и потом добавить ссылку на новый удаленный репозиторий..
    ```
...
- Что бы слить ветку fix в main,нужно перейти в вветку main и ввести команду merge fix.
  ...
  ```
> [!NOTE]
> классическое трехсторонее слияние (three-way merge). Стандартный алгоритм трехстороннего слияния применяется для двух файлов с общим предком. Условно этот алгоритм можно представить в виде следующих шагов:   
> поиск общего предка,  
...
- Что бы в ветку main залить данные через rebase.Идем в ветку которую хотим залить,вводим ребейс и имя ветку куда бедет перписывать комиты,потом возвращаемся в основную ветку и делаем мердж - что бы HEAD основной ветки переместился в конец.
  ...
  git rebase fix main
  ```
  - сложный ребейс.Если у нас есть ветка местер и потом в какой то момент от нее сделали ответвление в новую ветку server.Из самого первого комита в ветке сервер сделали ответвление в новую ветку client.Далее создавали комиты в ветках сервер client и server.И вот в конце нам надо только комиты клиента добавить в ветку мастер, для этого выполняем команду нижу.Команда говорит иди в ветку client  и
  найди изменения относительно ветки server и примени эти изменения на ветку master.
...
> [!Пояснение Merge]На github.com данная процедура делается следующим образом:
